...
- Мост читает length-prefixed JSON из stdin. Для `listProfiles` / `saveProfiles` читает или пишет `hosts.json`; иначе делает HTTP POST на `host:port/` с телом `{ "url": ..., "action": ..., "params": [...] }`.
- По умолчанию мост использует порт `7531` (и расширение также настроено на 7531).
...
Профили хранятся в `~/.config/post2mpv/hosts.json` (или `$XDG_CONFIG_HOME/post2mpv/hosts.json`). Файл читает и пишет нативный мост. Расширение в файловую систему не ходит.

Если файла нет — список профилей пустой, расширение не падает. Одиночный пункт контекстного меню «Play in MPV» использует `http://127.0.0.1:7531`. Файл и каталог создаются при первом сохранении из настроек расширения; существующий `post2mpv.conf` не трогается. Битый JSON — ошибка в настройках, файл молча не перезаписывается.
...
- После обновления расширения пересоберите мост (`go build … post2mpv-bridge.go`) — старый бинарник не умеет `listProfiles` / `saveProfiles`.
- Нет профилей и нет `hosts.json`: так и задумано, пока не сохраните профиль в настройках.
- Битый `hosts.json`: настройки покажут ошибку и не перезапишут файл; исправьте JSON вручную.
